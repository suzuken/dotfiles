...
経営資料は「1 つの HTML に全部書く」と最初は速いが、数千行になると編集・検索・部分差し替えが重くなり、終盤の構造変更 (順序入れ替え・全面リスキン) が一括大手術になる。**だから最初から parts に分割し、build で結合する**。分割は「振る舞いを変えない可逆リファクタ」なので、閾値を待たず初日に行ってよい。

逆に、**ブランド (色・フォント・装飾) は最後に回す**のが正しい。内容が 8 割固まるまでは中立トークンで進め、終盤に CSS 変数を差し替えるだけでブランドが乗る。内容を色に従属させない。
...
build 後、配布前に必ず確認する (手順は `references/verification.md`):
...
これらは散文でなく `just audit` (build script の lint + Playwright 監査) に落とすと、毎回の遵守がプロンプト頼みでなく機械保証になる。
